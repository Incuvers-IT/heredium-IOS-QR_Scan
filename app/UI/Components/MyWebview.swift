...
struct MyWebview: UIViewRepresentable {
    ...
    func creatWKWebConfig() -> WKWebViewConfiguration {
        ...
        // 웹뷰 유저 컨트롤러(getWifiInfo)
        let userContentController = WKUserContentController()
        //userContentController.add(self.makeCoordinator(), name: "callbackHandler")
        userContentController.add(self.makeCoordinator(), name: "getWifiInfo")
        wkWebConfig.applicationNameForUserAgent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_4) AppleWebKit/600.7.12 (KHTML, like Gecko) Version/8.0.7 Safari/600.7.12"
...
extension MyWebview.Coordinator : WKUIDelegate {
    func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping () -> Void) {
        ...
        self.myWebView.viewModel.alertEvent.send(AlertModel(message, "확인", "", .alert))
        completionHandler()
    }
...
//            }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        default:
            ...
            //print("들어온 도메인 url.host: \(String(describing: url.host)) \(webView.url.host)")
            if url.scheme != "http" && url.scheme != "https" {
                ...
                print("aaa \(url)")
                print("aaa \(url.host)")
                print("aaa \(navigationAction.targetFrame)")
                if(navigationAction.targetFrame?.isMainFrame != nil){
                    decisionHandler(.allow)
                }/*else if(webView.url != nil && url.host == webView.url!.host){
                    decisionHandler(.allow)
                }*/else{
                    UIApplication.shared.open(url, options: [:], completionHandler: nil)
                    decisionHandler(.cancel)
...
//            }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        print("webView didFinish", myWebView.viewModel.state)
        ...
        //if(myWebView.viewModel.state == .first || myWebView.viewModel.state == .second){
        if(myWebView.viewModel.isFirst){
            myWebView.viewModel.isFirst = false
            ...
            myWebView
              .viewModel
              .nativeToGetWifiInfoResultEvent
              .sink{ message in
                  print("didFinish() called / nativeToJsEvent 이벤트 들어옴 / message: \(message)")
                  webView.evaluateJavaScript("getWifiInfoResult('\(message)');", completionHandler: { (result, error) in
            ...
            myWebView
              .viewModel
              .nativeToPermissionDeniedEvent
              .sink{ message in
                  print("didFinish() called / nativeToJsEvent 이벤트 들어옴 / message: \(message)")
                  webView.evaluateJavaScript("permissionDenied('\(message)');", completionHandler: { (result, error) in
                      if let result = result {
                          print("nativeToJs result 성공 : \(result)")
                      }
                      if let error = error {
...
extension MyWebview.Coordinator : WKScriptMessageHandler {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        print("WKWebViewCoordinator - userContentController / message: \(message.name)")
        
        if message.name == "getWifiInfo" {
            let date = Date()
            print("JSON 데이터가 웹으로부터 옴: \(message.body), \(date)")
            myWebView.viewModel.jsToNativeEvent.send(message.name)
        
            //DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                //self.messageToWebview(msg: "hello, I got your messsage: at \(date)")
            //}
        }
